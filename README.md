...
## Evaluate audio skims with objective faithfulness and highlightness metrics (AUDIO ONLY)

To evaluate a set of audio skims using the proposed objective faithfulness and highlightness metrics, first create a directory containing all summaries generated by the method you want to evaluate:

```text
/dir/to/summaries/
```

Each summary filename must contain its corresponding trade-off parameter (`z` in the paper) using the format `scenXXX`, where `XXX` is an integer between `000` and `100`.
...
summary_scen000.wav
summary_scen050.wav
this_is_my_scen100_summary.wav
```

Here, `scen050` corresponds to a trade-off value of `z = 0.5`.

The highlightness metric requires BEATs logits computed on the original audio dataset. Generate them with:

```
python get_logits.py /path/to/audio.h5 /path/to/logits.h5
```

Once the embeddings, logits, clustering information, and summary files are available, compute the 
faithfulness, highlighness, auc and z-correlation metrics with:

```
python get_metrics.py /path/to/embeddings.h5 /path/to/logits.h5 /path/to/dtw_metric.csv /path/to/max_sim_metric.csv /dir/to/summaries/ /path/to/output_embeddings.h5 /path/to/clusters.csv 
```
