...
def pack_waveforms_to_hdf5(args):
    ...
    csv_path = args.audio_dataset + '/meta.csv' 
    ...
    df['full_identifier'] = df['filename'].str.split('-').str[2:4].str.join('-')

    #random shuffle so that the samples are select are random
    df = df.sample(frac=1, random_state=42).reset_index(drop=True)
    test_set = df.groupby('identifier').first().reset_index(drop=True)    
    test_set = test_set.head(int(len(df)*(eval_ratio + valid_ratio)))

    df_train = df.drop(test_set.index)
    df_valid = test_set.sample(frac=valid_ratio/(eval_ratio+valid_ratio), random_state=42)
    df_eval = test_set.drop(df_valid.index)

    train_dataset = df_train['filename'].values
    valid_dataset = df_valid['filename'].values
    eval_dataset = df_eval['filename'].values
...
if __name__ == '__main__':
    ...
    parser.add_argument('--audio_dataset', type=str, default='/media/user/MT-SSD-NEW/0-PROJETS_INFO/Thèse/TAU-urban-acoustic-scenes-2020-mobile-development/', help='Directory where downloaded audio is saved.')
    parser.add_argument('--waveforms_hdf5_path', type=str, default='/media/user/MT-SSD-NEW/0-PROJETS_INFO/Thèse/6-diffusionFromThirdo-Data/spectral_data/', help='Path to save out packed hdf5.')
    parser.add_argument('--valid_ratio', type=float, default=0.000,
